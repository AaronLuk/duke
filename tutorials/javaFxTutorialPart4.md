...
1. Components are heavily dependent on each other:
   Why does `UI.Main` need to know that `UI.DialogBox` needs a `Label`? 
...
1. The code is untidy and long:
   ...
   The `UI.Main` class attempts to do it all. 
...
As part of the effort to separate the code handling Duke.Duke's logic and Duke.Duke.UI, let's _refactor_ the Duke.Duke.UI-related code to its own class.
We call these Duke.Duke.UI classes _controllers_. 
...
Without the annotation, we will have to make everything `public` and expose our Duke.Duke.UI to unwanted changes.
...
Let's create a new `UI.Main` class as the bridge between the existing logic in `Duke.Duke` and the Duke.Duke.UI in `UI.MainWindow`.
 ...
 */
public class UI.Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(UI.Main.class.getResource("/view/UI.MainWindow.fxml"));
...
The last change that we have to make is to point our `UI.Launcher` class in the right direction:
In `UI.Launcher.java`
```java
//...    
Application.launch(UI.Main.class, args);
//...
```
