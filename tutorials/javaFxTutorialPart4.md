...
As part of the effort to separate the code handling Duke's logic and Duke.UI, let's _refactor_ the Duke.UI-related code to its own class.
...
Let's create a new `Main` class as the bridge between the existing logic in `Duke` and the Duke.UI in `MainWindow`.
